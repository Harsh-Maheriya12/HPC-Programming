...
int main(int argc, char **argv) {
    ...
    for (int iter = 0; iter < Maxiter; iter++) {
        ...
        double iter_int = t1 - t0;
        double iter_norm = t2 - t1;
        double iter_move = t3 - t2;
        double iter_denorm = t4 - t3;
        double iter_total = t4 - t0;

        // Output for the benchmarking script to capture
        printf("ITER_DATA %d %lf %lf %lf %lf %lf\n", iter, iter_int, iter_norm, iter_move, iter_denorm, iter_total);

        total_int_time += iter_int;
        total_norm_time += iter_norm;
        total_move_time += iter_move;
        total_denorm_time += iter_denorm;
    }
