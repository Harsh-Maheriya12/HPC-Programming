...
void interpolation(double * __restrict mesh_value, Points * __restrict points, int n) {
...
// Implements the get_global_bounds logic (Normalization)
void get_global_bounds(double * __restrict mesh_value) {
...
void mover(double * __restrict mesh_value, Points * __restrict points, int n) {
    #pragma omp parallel for schedule(static)
    for (int i = 0; i < n; i++) {
        if (points[i].is_void) continue;
