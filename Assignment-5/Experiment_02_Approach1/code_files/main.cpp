...
int main() {
    ...
    printf("=== Experiment 02: Parallel Scalability (Approach 1: Deferred) ===\n");
    ...
    for (int g = 0; g < 3; g++) {
        ...
        for (int t = 0; t < 5; t++) {
            ...
            char filename[150];
            sprintf(filename, "parallel_grid%d_def_%dthreads.txt", g + 1, num_threads);
            FILE *out = fopen(filename, "w");

            fprintf(out, "=== Experiment 02: Parallel Scalability (Deferred) ===\n");
            ...
            for (int iter = 0; iter < Maxiter; iter++) {
                ...
                if (num_threads == 1) {
                    mover_serial_deferred(points, dx, dy, &del_count);
                } else {
                    mover_parallel_deferred(points, dx, dy, &del_count);
                }
